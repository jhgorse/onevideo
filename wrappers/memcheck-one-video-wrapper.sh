...
G_SLICE="always-malloc" G_DEBUG="$G_DEBUG,gc-friendly" GST_DEBUG="*:3,onevideo:6,$GST_DEBUG" \
	...
	valgrind --suppressions="${build_dir}"/tests/supp/gst.supp --tool=memcheck --leak-check=full --show-leak-kinds=definite,indirect --leak-resolution=high --num-callers=30 --log-file="${VALGRIND_LOG}" \
