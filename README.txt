...
3) Run ./one-video-cli.sh as follows:
...
* Call initiator (and participator peer):
 $ ./one-video-cli.sh -i lo -p 5000 -d /dev/video0 -c localhost:6000 # Add -c localhost:7000 etc as required
...
For memcheck and gdb, use ./memcheck-one-video-cli.sh or ./debug-one-video-cli.sh respectively
